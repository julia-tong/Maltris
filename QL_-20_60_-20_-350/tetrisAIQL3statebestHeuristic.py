...
rewards_map = {'inc_height': -20, 'clear_line': 60, 'holes': -20, 'top_height':-350}
...
QLfilename = 'maltrisQL3rstateTableBH.save'
QLgraphfilename = 'maltrisQL3rstateGraphDataBH.save'
...
class TetrisAI:
    ...
    def run(self, agent_host):
        ...
        while not done_update:
            ...
            print("self.pred_insta_drop2(next_action)", self.pred_insta_drop2(next_action))

            actions.append(self.normalize(self.pred_insta_drop2(next_action)))
            rewards.append(0)
            print("normalized actions", actions)
            T = sys.maxint
                    ...
                    if self.game.gameover == True:
                        ...
                        if game_overs == 5:
                            print("Best attempt, gamesplayed: ", self.gamesplayed,
                                  " avglvls: %.3f" % numpy.mean(self.listGameLvl),
                                  " avgclears: %.3f " % numpy.mean(self.listClears))
                            #self.saveQtable()
                            game_overs = 0
                            self.epsilon = 0
                        else:
